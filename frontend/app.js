...
function addResultActions(container, getText) {
...
function addLog(container, text, isReport) {
  ...
  if (isReport) addResultActions(container, () => text);
...
document.getElementById('mcpChatSendBtn')?.addEventListener('click', async (e) => {
  const btn = e.currentTarget;
  const message = document.getElementById('mcpChatPrompt')?.value?.trim();
  const out = document.getElementById('mcpChatResult');
  if (!out) return;
  out.style.display = 'block';
  if (!message) { out.textContent = '⚠️ Nhập yêu cầu trước.'; return; }
  const original = btn.textContent;
  btn.disabled = true; btn.textContent = '⏳ Đang xử lý (có thể mất chút thời gian nếu AI cần gọi nhiều công cụ)...';
  ...
  try {
    ...
    if (data.toolCalls?.length) {
      text += '\n\n— Đã gọi ' + data.toolCalls.length + ' công cụ: ' + data.toolCalls.map(t => t.name).join(', ');
    }
