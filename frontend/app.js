...
// ⚠️ FIX: model "3.5 Flash" (coding) KHÔNG còn nằm sẵn trong danh sách mặc định — trước đây nó
// nằm sẵn ở đây rồi mới bị GỠ RA sau (bất đồng bộ, sau khi xác minh không phải owner), nên nếu
// người dùng bình thường mở bảng chọn model NGAY LÚC TRANG VỪA TẢI (trước khi kịp gỡ), họ vẫn
// thấy và chọn được — dù backend luôn tự hạ xuống Smart cho non-owner, giao diện vẫn gây hiểu
// lầm là đang dùng model coding thật. Giờ danh sách mặc định KHÔNG CÓ coding; chỉ được THÊM VÀO
// sau khi /api/me xác nhận đúng là owner (xem applyOwnerRestrictions bên dưới) — an toàn theo
// hướng "mặc định ẩn, xác nhận mới hiện" thay vì "mặc định hiện, nghi ngờ mới ẩn".
const MODEL_OPTIONS = [
...
const CODING_MODEL_OPTION = { value: 'gemini-3.5-flash', label: '3.5 Flash', desc: 'Tối ưu cho việc viết/sửa code — chỉ chủ app' };
...
// Cờ TỰ QUẢN của app cho biết mình có thực sự vừa bắt đầu 1 lượt đọc hay chưa.
// Không dùng window.speechSynthesis.speaking làm nguồn sự thật duy nhất nữa, vì trên
// nhiều bản Android (Chrome/WebView) cờ này có bug nổi tiếng: đôi khi bị kẹt ở "true"
// mãi mãi dù không phát âm thanh nào, khiến nút "Đọc to" chỉ luôn chạy vào nhánh dừng
// (stopSpeaking) mà không bao giờ phát được giọng mới -> AI "không đọc" tin nhắn nào nữa.
let appIsSpeaking = false;
...
function speakWithBrowser(text, btnEl, onEnd) {
  ...
  // Luôn cancel() trước khi speak() mới — dọn sạch mọi utterance "ma" còn treo lại
  // từ trước (đây là nguyên nhân phổ biến khiến speaking bị kẹt true trên Android).
  ...
  utter.onend = () => { appIsSpeaking = false; if (btnEl) { btnEl.classList.remove('speaking'); btnEl.textContent = '🔊 Đọc to'; } onEnd?.(); };
  utter.onerror = () => { appIsSpeaking = false; if (btnEl) { btnEl.classList.remove('speaking'); btnEl.textContent = '🔊 Đọc to'; } onEnd?.(); };
  // Watchdog: nếu sau 15s mà speechSynthesis vẫn báo "đang nói" nhưng utterance đó
  // đáng lẽ đã xong (văn bản ngắn/trung bình), coi như engine bị treo -> tự reset để
  // lần bấm "Đọc to" tiếp theo không bị chặn vô thời hạn.
  setTimeout(() => {
    if (appIsSpeaking && !window.speechSynthesis.speaking) {
      appIsSpeaking = false;
      if (btnEl) { btnEl.classList.remove('speaking'); btnEl.textContent = '🔊 Đọc to'; }
    }
  }, 15000);
  // requestAnimationFrame kép: một số WebView Android cần UI "settle" trước khi speak()
  // thực sự bắt đầu phát, nếu gọi ngay lập tức đôi khi bị bỏ qua trong im lặng.
...
async function speakText(text, btnEl, opts = {}) {
  ...
  // Dùng appIsSpeaking (cờ tự quản) thay vì chỉ tin window.speechSynthesis.speaking,
  // vì cờ của trình duyệt có thể bị kẹt "true" trên Android dù không phát gì cả.
  if (currentAudioEl || appIsSpeaking) {
...
// ===================== CHAT =====================
// Copy + Đọc to dùng chung cho MỌI panel không phải Chat chính (Giải bài tập, Mở web & hỏi AI,
// Agent Mode, Code Editor, Tạo ảnh, Artifacts...) — để các panel đó có hành động giống hệt Chat
// thay vì chỉ hiện chữ trơn không thao tác được gì.
function addResultActions(container, getText, opts = {}) {
  ...
  speakBtn.addEventListener('click', () => speakText(getText(), speakBtn));
...
const ArtifactStore = {
  ...
  add({ title, code, source }) {
    const list = this.all();
    // Chống trùng lặp: nếu ĐÃ có artifact với code giống hệt (dù khác title/source), dùng lại
    // artifact cũ thay vì tạo mới — đây là lý do trước đây bị "artifacts trùng lặp": mỗi lần
    // renderHistory() render lại lịch sử chat cũ, code auto-tạo artifact lại chạy lại từ đầu.
...
function appendMsgToDOM(role, text, grounding, index) {
  ...
  // Nếu tách được artifact, hiện nút "💾 Lưu thành Artifact" — KHÔNG tự lưu ngay (trước đây tự
  // lưu ngay khi AI trả lời xong, dù người dùng chưa bấm gì — đúng lỗi "chưa bấm lưu mà đã vào
  // Artifacts"). Bấm nút mới thật sự gọi ArtifactStore.add() và hiện card như bình thường.
  if (artifactInfo) {
    const title = (before || text).replace(/\s+/g, ' ').slice(0, 60) || 'Artifact từ Chat';
    const saveBar = document.createElement('div');
    saveBar.className = 'msg-actions';
    const saveBtn = document.createElement('button');
    saveBtn.className = 'msg-action-btn';
    saveBtn.textContent = '💾 Lưu thành Artifact';
    saveBtn.addEventListener('click', () => {
      const artifact = ArtifactStore.add({ title, code: artifactInfo.code, source: 'chat' });
      saveBar.replaceWith(artifactCardEl(artifact));
    });
...
// Xử lý quay lại từ trang OAuth (GitHub/Google) sau khi người dùng đăng nhập/đồng ý xong —
// worker redirect về đây kèm ?mcp_connected=<provider> hoặc ?mcp_error=<lý do>.
(function handleMcpOAuthReturn() {
  ...
  switchMode('settings');
  setTimeout(() => {
    if (connected) alert(`✅ Đã kết nối ${connected} thành công!`);
    if (error) alert(`⚠️ Kết nối thất bại: ${decodeURIComponent(error)}`);
    invalidateMcpConnectedCache(); loadMcpConnections();
  }, 300);

  // Dọn query string khỏi URL để reload trang không hiện lại thông báo cũ.
...
(async function applyOwnerRestrictions() {
  try {
    ...
    const data = await r.json();
    window.__isOwner = !!data.isOwner;

    if (data.isOwner) {
      // Chủ app -> thêm model coding vào bảng chọn model của Chat chính (mặc định KHÔNG có,
      // xem khai báo MODEL_OPTIONS phía trên) — chỉ thêm sau khi XÁC NHẬN xong, không phải
      // "có sẵn rồi gỡ" như trước, để tránh khoảng hở trước khi xác minh xong.
      if (!MODEL_OPTIONS.some(m => m.value === CODING_MODEL_OPTION.value)) {
        MODEL_OPTIONS.splice(3, 0, CODING_MODEL_OPTION); // chèn giữa "3 Flash" và "3.1 Pro" cho đúng thứ tự cũ
      }

      // Thêm lựa chọn model coding vào dropdown "Thử gọi API bằng key" trong Cài đặt (dropdown
      // này còn dùng để xuất code gắn vào file khác, nên chủ app cũng xuất được code dùng model coding).
      const sel = document.getElementById('apiKeyTestModel');
      if (sel && !sel.querySelector('option[value="coding"]')) {
        const opt = document.createElement('option');
        opt.value = 'coding';
        opt.textContent = 'Coding (tối ưu viết/sửa code) — chỉ chủ app';
        sel.appendChild(opt);
      }
      return; // giữ nguyên các phần còn lại, không ẩn gì
    }

    document.querySelector('.nav-btn[data-mode="agent"]')?.remove();
    document.querySelector('.nav-btn[data-mode="webask"]')?.remove();
  } catch (e) {
    // Không tải được /api/me (mất mạng, chưa deploy...) -> cứ để nguyên UI (KHÔNG có coding vì
    // đó là mặc định an toàn), server vẫn tự chặn nếu có ai cố lách bằng cách khác.
  }
})();
...
// Xoá sạch state/nội dung của mọi panel khác ngoài Chat, để "New Chat" thực sự
// bắt đầu lại từ đầu dù người dùng đang ở Image / Video / Agent / Artifacts / Code Editor.
// Trước đây switchMode('chat') chỉ đổi panel hiển thị, không xoá dữ liệu cũ của các panel kia
// -> quay lại các panel đó vẫn thấy kết quả/log của phiên trước, tưởng như "không tạo hội thoại mới".
function resetAllPanels() {
  // Image
  const imgResults = document.getElementById('imgResults');
  if (imgResults) imgResults.innerHTML = '';
  const imgPrompt = document.getElementById('imgPrompt');
  if (imgPrompt) imgPrompt.value = '';

  // Video
  const vidJobs = document.getElementById('vidJobs');
  if (vidJobs) vidJobs.innerHTML = '';
  const vidResults = document.getElementById('vidResults');
  if (vidResults) vidResults.innerHTML = '';
  ...
  // Web Ask / tìm kiếm
  const webaskResults = document.getElementById('webaskResults');
  if (webaskResults) webaskResults.innerHTML = '';

  // Lưu ý: KHÔNG xoá panel-artifacts và panel-library — đó là kho lưu trữ lâu dài
  // của người dùng (artifact/file đã tạo), không phải trạng thái của 1 phiên chat.
}
...
// ===================== @ MENTION MCP (nhắc AI dùng đúng công cụ đã kết nối) =====================
let mcpConnectedCache = null; // [{key,label}] — cache để không gọi API mỗi lần gõ phím
async function getMcpConnectedList() {
  if (mcpConnectedCache) return mcpConnectedCache;
  try {
    const r = await apiFetch(API_BASE + '/api/mcp/status');
    const data = await r.json();
    mcpConnectedCache = (data.servers || []).filter(s => s.connected).map(s => ({ key: s.key, label: s.label }));
  } catch (e) { mcpConnectedCache = []; }
  return mcpConnectedCache;
}
// Xoá cache khi danh sách kết nối thay đổi (kết nối/ngắt kết nối trong Cài đặt).
function invalidateMcpConnectedCache() { mcpConnectedCache = null; }

// Trả về {start, query} nếu con trỏ đang ở ngay sau 1 "@..." chưa có dấu cách — dùng để hiện gợi ý.
function findMentionAtCursor(text, cursorPos) {
  const upToCursor = text.slice(0, cursorPos);
  const m = upToCursor.match(/(?:^|\s)@([a-zA-Z0-9_]*)$/);
  if (!m) return null;
  return { start: cursorPos - m[1].length - 1, query: m[1].toLowerCase() };
}

async function updateMcpMentionPopup() {
  const popup = document.getElementById('mcpMentionPopup');
  if (!popup) return;
  const text = els.chatInput.value;
  const cursorPos = els.chatInput.selectionStart;
  const mention = findMentionAtCursor(text, cursorPos);
  if (!mention) { popup.classList.add('hidden'); popup.innerHTML = ''; return; }

  const list = await getMcpConnectedList();
  const filtered = list.filter(s => s.key.startsWith(mention.query));
  if (!filtered.length) { popup.classList.add('hidden'); popup.innerHTML = ''; return; }

  popup.innerHTML = '';
  filtered.forEach(s => {
    const item = document.createElement('div');
    item.textContent = '@' + s.key + '  (' + s.label + ')';
    item.className = 'mcp-mention-item';
    item.style.whiteSpace = 'nowrap';
    item.addEventListener('mousedown', (e) => {
      // mousedown (không phải click) để chạy TRƯỚC khi textarea mất focus/blur.
      e.preventDefault();
      const before = text.slice(0, mention.start);
      const after = text.slice(cursorPos);
      els.chatInput.value = before + '@' + s.key + ' ' + after;
      const newPos = (before + '@' + s.key + ' ').length;
...
// Nếu tin nhắn bắt đầu bằng "@<provider> " và provider đó đã kết nối -> trả về {provider, message}
// (message đã bỏ phần "@provider " ở đầu) để route sang MCP thay vì chat thường.
async function extractMcpMention(text) {
  const m = text.match(/^@([a-zA-Z0-9_]+)\s+([\s\S]*)$/);
  if (!m) return null;
  const list = await getMcpConnectedList();
  const found = list.find(s => s.key === m[1].toLowerCase());
  if (!found) return null;
  return { provider: found, message: m[2].trim() };
}
...
function sendChat() {
  ...
  document.getElementById('mcpMentionPopup')?.classList.add('hidden');
  if (state.deepResearchMode) {
    setDeepResearchMode(false);
    runDeepResearchAsChatMessage(text);
    return;
  }
  extractMcpMention(text).then(mention => {
    if (mention) { submitMcpMentionMessage(text, mention.provider, mention.message); return; }
...
// Gửi 1 tin nhắn qua đúng MCP server được nhắc tới (@github, @cloudflare...) — không stream, vì
// AI có thể cần gọi công cụ nhiều vòng trước khi trả lời xong (xem worker/src/mcpChat.js).
async function submitMcpMentionMessage(rawText, provider, message) {
  const index = state.history.length;
  appendMsgToDOM('user', rawText, null, index);
  state.history.push({ role: 'user', text: rawText });
  if (!state.temp) {
    await ensureConversation(rawText);
    if (state.conversationId) saveMessageToServer('user', rawText);
  }
  const assistantDiv = appendMsgToDOM('assistant', '', null, index + 1);
  assistantDiv.innerHTML = `<span class="hint">🔌 Đang dùng công cụ ${provider.label}...</span>`;
  try {
    const r = await apiFetch(API_BASE + '/api/mcp/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message }) });
    const data = await r.json();
    if (!r.ok) throw new Error(data.error || 'Lỗi MCP');
    await handleMcpChatResult(data, assistantDiv, rawText, provider, index);
  } catch (e) {
    assistantDiv.innerHTML = renderMarkdown('⚠️ Lỗi: ' + e.message);
  }
}

// Render kết quả từ /api/mcp/chat HOẶC /api/mcp/chat/confirm — 2 dạng có thể xảy ra:
//   1) trả lời xong bình thường (data.text)
//   2) đang chờ xác nhận 1 hành động không thể hoàn tác (data.needsConfirmation) — hiển thị hộp
//      cảnh báo + 2 nút Đồng ý/Huỷ, chỉ gọi tool thật khi người dùng bấm Đồng ý.
async function handleMcpChatResult(data, assistantDiv, rawText, provider, index) {
  if (data.needsConfirmation) {
    const argsJson = data.args ? escapeHtmlText(JSON.stringify(data.args, null, 2)) : '';
    assistantDiv.innerHTML = `
      <div class="mcp-confirm-box" style="border:1px solid var(--danger,#e5484d);border-radius:10px;padding:12px 14px;">
        <div style="font-weight:600;margin-bottom:6px;">⚠️ Cần xác nhận hành động không thể hoàn tác</div>
        <div style="margin-bottom:10px;white-space:pre-wrap;">${renderMarkdown(data.description || data.tool)}</div>
        ${argsJson ? `
        <details style="margin-bottom:10px;">
          <summary style="cursor:pointer;font-size:0.85em;opacity:0.75;">Xem chi tiết tham số gốc (${data.tool})</summary>
          <pre style="font-size:0.8em;overflow-x:auto;background:rgba(128,128,128,0.1);padding:8px;border-radius:6px;margin-top:6px;">${argsJson}</pre>
        </details>` : ''}
        <div style="display:flex;gap:8px;">
          <button type="button" class="mcp-confirm-approve" style="padding:6px 14px;border-radius:8px;background:var(--danger,#e5484d);color:#fff;border:none;cursor:pointer;">Đồng ý thực hiện</button>
          <button type="button" class="mcp-confirm-reject" style="padding:6px 14px;border-radius:8px;background:transparent;border:1px solid var(--border,#888);cursor:pointer;">Huỷ</button>
        </div>
      </div>`;
    const approveBtn = assistantDiv.querySelector('.mcp-confirm-approve');
    const rejectBtn = assistantDiv.querySelector('.mcp-confirm-reject');
    const onClick = (approve) => async () => {
      approveBtn.disabled = true; rejectBtn.disabled = true;
      assistantDiv.innerHTML = `<span class="hint">🔌 ${approve ? 'Đang thực hiện...' : 'Đang huỷ...'}</span>`;
      try {
        const r2 = await apiFetch(API_BASE + '/api/mcp/chat/confirm', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ confirmId: data.confirmId, approve }),
        });
        const data2 = await r2.json();
        if (!r2.ok) throw new Error(data2.error || 'Lỗi MCP');
        await handleMcpChatResult(data2, assistantDiv, rawText, provider, index);
      } catch (e) {
        assistantDiv.innerHTML = renderMarkdown('⚠️ Lỗi: ' + e.message);
      }
    };
    approveBtn.addEventListener('click', onClick(true));
    rejectBtn.addEventListener('click', onClick(false));
    return;
  }

  let text = data.text || '(AI không trả về nội dung)';
  if (data.toolCalls?.length) text += `\n\n_(đã gọi ${data.toolCalls.length} công cụ: ${data.toolCalls.map(t => t.name).join(', ')})_`;
  assistantDiv.innerHTML = renderMarkdown(text);
  enhanceCodeBlocks(assistantDiv);
  addResultActions(assistantDiv.parentElement, () => text, {
    onEdit: () => { els.chatInput.value = rawText; els.chatInput.focus(); },
  });
  state.history.push({ role: 'assistant', text });
  saveHistory();
  if (!state.temp && state.conversationId) saveMessageToServer('assistant', text, '@' + provider.key);
}
...
async function hwSolve() {
  ...
  // Ghi lại vào lịch sử hội thoại (D1) như Chat chính — trước đây panel này không lưu gì cả,
  // nên không hiện trong 🗂️ Thư viện / sidebar lịch sử.
  await ensureConversation('📘 ' + (text || 'Giải bài tập'));
  saveMessageToServer('user', text || '(kèm ảnh đề bài)', 'auto');
  ...
  await streamPost(API_BASE + '/api/chat/stream', {
    ...
    error: (e) => { hw.els.solutionBody.innerHTML = renderMarkdown('⚠️ Lỗi: ' + (typeof e === 'string' ? e : JSON.stringify(e))); },
      ...
      hw.solution = full;
      enhanceCodeBlocks(hw.els.solutionBody);
      addResultActions(hw.els.solutionBody.parentElement || hw.els.solutionBody, () => hw.solution, {
        onEdit: () => { hw.els.input.value = text; hw.els.input.focus(); },
      });
      saveMessageToServer('assistant', full, 'auto');
    },
  });
}
...
document.getElementById('imgGenBtn').addEventListener('click', async () => {
  ...
  await ensureConversation('🖼️ ' + prompt);
  saveMessageToServer('user', prompt, 'auto');
  try {
    ...
    if (data.text) {
      ...
      box.appendChild(p);
      addResultActions(box, () => data.text, {
        onEdit: () => { document.getElementById('imgPrompt').value = prompt; document.getElementById('imgPrompt').focus(); },
      });
    }
    if (data.images.some(img => img.savedUrl)) {
      box.insertAdjacentHTML('beforeend', '<p class="hint">✅ Đã lưu vào 🗂️ Thư viện.</p>');
    }
    saveMessageToServer('assistant', data.text || '(đã tạo ảnh)', 'auto');
...
(function setupWebAsk() {
  ...
  askBtn.addEventListener('click', async () => {
    ...
    await ensureConversation('🧭 ' + (question || url));
    saveMessageToServer('user', (question ? question + '\n' : '') + url, 'auto');
    ...
    try {
      ...
      addResultActions(card, () => data.answer || '', {
        onEdit: () => { questionInput.value = question; questionInput.focus(); },
      });
      saveMessageToServer('assistant', data.answer || '', 'auto');
    } catch (e) {
      clearTimeout(timeoutId);
...
document.getElementById('vidGenBtn').addEventListener('click', async () => {
  ...
  editBtn.addEventListener('click', () => { document.getElementById('vidPrompt').value = prompt; document.getElementById('vidPrompt').focus(); });
...
function extractArtifactFromText(text) {
  ...
  // Trước đây HTML LUÔN được coi là artifact bất kể ngắn dài — 1 đoạn HTML 2-3 dòng cũng bị biến
  // thành artifact. Giờ áp cùng luật >=6 dòng như các ngôn ngữ khác, trừ khi rõ ràng là 1 trang
  // đầy đủ (<!DOCTYPE hoặc <html>) thì luôn tách ra dù ngắn, vì đó chắc chắn là thứ cần chạy riêng.
  if (htmlMatch) {
    const code = htmlMatch[1].trim();
    const looksLikeFullPage = /<!DOCTYPE|<html[\s>]/i.test(code);
    if (looksLikeFullPage || code.split('\n').length >= 6) return { code, lang: 'html' };
  }
...
artifactEls.genBtn.addEventListener('click', async () => {
  ...
  await streamPost(API_BASE + '/api/chat/stream', {
    ...
    chunk: t => { full += t; },
      ...
      saveMessageToServer('assistant', '```html\n' + code + '\n```', 'auto');
      artifactEls.genBtn.disabled = false;
      artifactEls.genBtn.textContent = '⚡ Tạo Artifact mới';
      // Không xoá prompt sau khi tạo xong (trước đây tự xoá luôn) — để người dùng có thể SỬA
      // ngay trên prompt vừa dùng rồi bấm tạo lại, thay vì phải gõ lại từ đầu.
...
document.getElementById('codeAskBtn').addEventListener('click', async () => {
  ...
  await ensureConversation('💻 ' + instruction);
  saveMessageToServer('user', instruction, state.model);
  ...
  await streamPost(API_BASE + '/api/chat/stream', {
    ...
    aborted: () => {
      ...
      const { explanation, code } = splitCodeAndExplanation(fullText);
      assistantBubble.innerHTML = renderMarkdown(explanation);
      addResultActions(assistantBubble.parentElement, () => fullText, {
        onEdit: () => { document.getElementById('codeInstruction').value = instruction; document.getElementById('codeInstruction').focus(); },
      });
      codeEditorHistory.push({ role: 'assistant', text: fullText });
      saveMessageToServer('assistant', fullText, state.model);
      if (code) {
        editor.value = code;
        // Code đủ dài -> hiện nút lưu thành Artifact (không tự lưu ngay, để người dùng chủ động chọn)
        if (code.split('\n').length >= 6) {
          const saveBtn = document.createElement('button');
          saveBtn.className = 'msg-action-btn';
          saveBtn.textContent = '💾 Lưu thành Artifact';
          saveBtn.addEventListener('click', () => {
            const artifact = ArtifactStore.add({ title: instruction.slice(0, 60), code, source: 'code' });
            saveBtn.replaceWith(artifactCardEl(artifact));
          });
          assistantBubble.parentElement.appendChild(saveBtn);
        }
        refreshCodePreview();
      }
    },
...
// ===================== VOICE (chế độ trò chuyện liên tục — tự nghe lại sau khi AI nói xong) =====================
// ⚠️ FIX "voice tự tắt, không như dạng stream live": TRƯỚC ĐÂY mỗi lượt chỉ ghi âm 1 lần rồi
// DỪNG HẲN (phải tự bấm nút để nói lượt tiếp theo) — cảm giác như voice "tự tắt" sau mỗi câu.
// Giờ: sau khi AI nói xong câu trả lời, mic TỰ ĐỘNG mở lại để nghe lượt tiếp theo (như đang
// gọi điện/trò chuyện liên tục), không cần bấm lại nút mỗi lần. Bấm nút bất cứ lúc nào để:
//   - đang nghe -> dừng ghi âm, gửi đi
//   - AI đang nói -> ngắt lời AI, dừng hẳn vòng lặp (không tự nghe lại nữa)
let voiceRecorder, voiceChunks = [];
let voiceLiveMode = false; // true = đang trong vòng lặp trò chuyện liên tục, tự nghe lại sau mỗi câu
...
async function startVoiceRecording() {
  ...
  voiceRecorder.onstop = async () => {
    ...
    await streamPost(API_BASE + '/api/chat/stream', {
      ...
      chunk: t => { full += t; },
        ...
        speakText(replyText, null, {
          ...
          onEnd: () => {
            ...
            if (voiceLiveMode) {
              // Tự nghe lại lượt tiếp theo — đây chính là phần "sống" (live) của cuộc trò chuyện.
              startVoiceRecording();
            } else {
      ...
      error: e => {
        userLine.textContent = '🎤 Bạn: (đoạn ghi âm)';
        voiceStatus.textContent = '⚠️ Lỗi: ' + e;
        voiceLiveMode = false;
...
voiceBtn.addEventListener('click', async () => {
  // AI đang nói -> bấm để NGẮT LỜI, dừng hẳn vòng lặp trò chuyện liên tục.
  if (voiceBtn.classList.contains('speaking')) {
    voiceLiveMode = false;
    voiceAudio.pause();
    voiceAudio.currentTime = 0;
    voiceBtn.classList.remove('speaking');
    voiceStatus.textContent = 'Đã ngắt — nhấn để nói';
    return;
  }
  // Đang ghi âm -> bấm để dừng và gửi đi.
  ...
  // Đang rảnh -> bắt đầu vòng lặp trò chuyện liên tục.
  voiceLiveMode = true;
...
document.getElementById('agentRunBtn').addEventListener('click', async () => {
  ...
  saveMessageToServer('user', task, 'auto');
  const handlers = agentModeHandleStream(log, runBtn);
  const origFinal = handlers.final;
  handlers.final = (text) => { origFinal(text); saveMessageToServer('assistant', text || '', 'auto'); };
  streamPost(API_BASE + '/api/agent/run', { task }, handlers);
});
...
function addLog(container, text, isReport) {
  ...
  // Chỉ thêm Copy/Đọc to cho dòng KẾT QUẢ CUỐI CÙNG (report) — không thêm cho từng dòng
  // "đang suy nghĩ/đang dùng công cụ" vì sẽ rất rối, không ai cần copy/đọc mấy dòng đó.
  // (Không thêm "Sửa" ở đây: dòng report là kết quả cuối, không phải nhiệm vụ gốc — sửa nhầm nội
  // dung report vào ô nhiệm vụ sẽ gây khó hiểu hơn là hữu ích.)
  if (isReport) {
    addResultActions(container, () => text, {
      onEdit: () => { document.getElementById('agentTask').value = lastAgentTask || ''; document.getElementById('agentTask').focus(); },
    });
  }

  // Nếu đây là báo cáo cuối cùng và chứa 1 khối code đáng kể, hiện nút để LƯU (không tự lưu ngay).
  if (isReport) {
...
async function loadApiKey() {
  ...
  try {
    ...
    if (msg && typeof data.remaining === 'number') {
      msg.textContent = `Còn ${data.remaining}/2 lượt tạo key mới hôm nay.`;
    }
  } catch (e) {
...
// ===================== MCP — kết nối tài khoản riêng (GitHub, Cloudflare...) =====================
// Danh sách provider MCP người dùng ĐÃ kết nối — dùng để gợi ý @mention trong khung chat chính.
let connectedMcpProviders = [];
async function refreshConnectedMcpProviders() {
  try {
    const r = await apiFetch(API_BASE + '/api/mcp/status');
    const data = await r.json();
    connectedMcpProviders = (data.servers || []).filter(s => s.connected).map(s => s.key);
  } catch (e) { /* chưa đăng nhập hoặc lỗi mạng -> coi như chưa kết nối gì, không chặn chat */ }
}
if (localStorage.getItem(AUTH_KEY)) refreshConnectedMcpProviders();

async function loadMcpConnections() {
  refreshConnectedMcpProviders();
  const box = document.getElementById('mcpConnectionsList');
  if (!box) return;
  box.innerHTML = '<p class="hint">Đang tải...</p>';
  try {
    const r = await apiFetch(API_BASE + '/api/mcp/status');
    const data = await r.json();
    if (!r.ok) throw new Error(data.error || 'Lỗi tải danh sách');
    box.innerHTML = '';
    data.servers.forEach(s => {
      const row = document.createElement('div');
      row.style.cssText = 'display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:8px;border:1px solid var(--border,#2a2f3a);border-radius:8px';
      if (s.authType === 'oauth') {
        if (s.connected) {
          row.innerHTML = `<strong style="min-width:110px">${s.label}</strong><span style="color:var(--ok,#3ddc84)">✅ Đã kết nối</span>`;
          const btn = document.createElement('button');
          btn.textContent = '🔌 Ngắt kết nối';
          btn.addEventListener('click', async () => {
            await apiFetch(API_BASE + '/api/mcp/disconnect', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ provider: s.key }) });
            invalidateMcpConnectedCache(); loadMcpConnections();
          });
          row.appendChild(btn);
        } else {
          row.innerHTML = `<strong style="min-width:110px">${s.label}</strong>`;
          const btn = document.createElement('button');
          btn.textContent = '🔗 Kết nối bằng OAuth';
          btn.addEventListener('click', () => {
            const token = localStorage.getItem(AUTH_KEY);
            if (!token) { alert('Bạn cần đăng nhập trước.'); return; }
            window.location.href = `${API_BASE}/api/mcp/oauth/start?provider=${s.key}&token=${encodeURIComponent(token)}`;
          });
          row.appendChild(btn);
        }
        box.appendChild(row);
      ...
      if (s.connected) {
        ...
        input.type = 'password';
        input.placeholder = s.tokenHint || 'Dán token...';
        input.style.flex = '1';
        ...
        btn.addEventListener('click', async () => {
          ...
          try {
            const r2 = await apiFetch(API_BASE + '/api/mcp/connect', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ provider: s.key, token }) });
    ...
    box.innerHTML = '<p class="hint">⚠️ ' + e.message + '</p>';
  }
}
document.querySelector('.nav-btn[data-mode="settings"]')?.addEventListener('click', loadMcpConnections);
...
document.getElementById('regenApiKeyBtn')?.addEventListener('click', async () => {
  if (!confirm('Key cũ sẽ ngừng hoạt động ngay. File HTML/app nào đang dùng key cũ sẽ phải cập nhật lại. Bạn chỉ được tạo key mới tối đa 2 lần/ngày. Tiếp tục?')) return;
  const msg = document.getElementById('apiKeyMsg');
  try {
    ...
    if (msg) msg.textContent = `✅ Đã tạo key mới. Còn ${data.remaining}/2 lượt hôm nay.`;
  } catch (e) {
...
document.getElementById('apiKeyTestSendBtn')?.addEventListener('click', async (e) => {
  ...
  const model = document.getElementById('apiKeyTestModel')?.value || 'smart';
...
document.getElementById('apiKeyGenSnippetBtn')?.addEventListener('click', () => {
  ...
  code.textContent =
`<script>
async function askVelocitix(prompt, model = '${model}') {
  const r = await fetch('${API_BASE}/api/external/chat', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-api-key': '${apiKey}' },
    body: JSON.stringify({ prompt, model })
...
// model: 'smart' (mặc định, chất lượng tốt) | 'fast' (nhanh) | 'lite' (ngắn gọn, nhanh nhất)
<\/script>`;
