...
async function loadApiKey() {
  ...
  try {
    ...
    if (msg && typeof data.remaining === 'number') {
      msg.textContent = `Còn ${data.remaining}/2 lượt tạo key mới.`;
    }
  } catch (e) {
...
document.getElementById('regenApiKeyBtn')?.addEventListener('click', async () => {
  if (!confirm('Key cũ sẽ ngừng hoạt động ngay. File HTML/app nào đang dùng key cũ sẽ phải cập nhật lại. Bạn chỉ được tạo key mới tối đa 2 lần. Tiếp tục?')) return;
  const msg = document.getElementById('apiKeyMsg');
  try {
    ...
    if (msg) msg.textContent = `✅ Đã tạo key mới. Còn ${data.remaining}/2 lượt.`;
  } catch (e) {
