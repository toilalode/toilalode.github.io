...
(function setupWebAsk() {
  ...
  askBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const data = await r.json();
      if (data.error) {
        card.innerHTML = `<div class="webask-src">🔗 ${url}</div><p class="hint">⚠️ ${data.error}</p>`;
        return;
      }
      card.innerHTML = `<div class="webask-src">🔗 ${data.sourceUrl || url}</div>${renderMarkdown(data.answer || '')}`;
    } catch (e) {
      card.innerHTML = `<div class="webask-src">🔗 ${url}</div><p class="hint">Lỗi: ${e.message}</p>`;
    }
  });
})();
