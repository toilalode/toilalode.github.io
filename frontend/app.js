...
(async function applyOwnerRestrictions() {
  try {
    ...
    if (data.isOwner) {
      // Chủ app -> thêm lựa chọn model coding vào dropdown "Thử gọi API bằng key" trong Cài đặt
      // (dropdown này còn dùng để xuất code gắn vào file khác, nên chủ app cũng xuất được code
      // dùng model coding).
...
async function updateMcpMentionPopup() {
  ...
  filtered.forEach(s => {
    ...
    item.addEventListener('mouseenter', () => item.style.background = 'var(--hover,#232833)');
...
async function loadMcpConnections() {
  ...
  try {
    ...
    data.servers.forEach(s => {
      ...
      if (s.authType === 'oauth_pending') {
        row.innerHTML = `<strong style="min-width:110px">${s.label}</strong><span class="hint">🔒 ${s.note} — chưa hỗ trợ</span>`;
        box.appendChild(row);
        return;
      }
      if (s.authType === 'oauth') {
