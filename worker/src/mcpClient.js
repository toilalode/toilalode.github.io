...
// ⚠️ FIX "ngốn token": 1 số MCP server (đặc biệt Cloudflare) trả về HÀNG CHỤC-HÀNG TRĂM tool bao
// phủ toàn bộ API của họ. Gửi NGUYÊN XI toàn bộ danh sách đó cho Gemini ở MỖI lượt gọi trong vòng
// lặp function-calling (có thể tới MAX_TOOL_ROUNDS lượt) làm phí token khủng khiếp — mỗi tool có
// tên + mô tả + schema tham số, nhân với hàng trăm tool là rất nhiều chữ lặp lại y hệt mỗi lượt.
// Giảm bằng 2 cách: (1) cắt mô tả ngắn lại nhiều (300 thay vì 1000 ký tự — Gemini vẫn hiểu được ý
// chính của tool để quyết định có gọi hay không, không cần mô tả dài dòng), (2) giới hạn tổng số
// tool gửi đi (mặc định 60) — nếu 1 server có nhiều hơn, chỉ lấy 60 cái đầu tiên server trả về
// (thường là những tool phổ biến/cơ bản nhất theo thứ tự server tự sắp).
const MAX_TOOLS_PER_SERVER = 60;
const MAX_TOOL_DESCRIPTION_LEN = 300;

function mcpToolsToGeminiDeclarations(mcpTools) {
  return mcpTools.slice(0, MAX_TOOLS_PER_SERVER).map(t => ({
    name: t.name,
    description: (t.description || '').slice(0, MAX_TOOL_DESCRIPTION_LEN),
