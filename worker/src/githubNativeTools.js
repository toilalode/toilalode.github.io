...
const GITHUB_NATIVE_TOOLS = [
  ...
  tool('commit_multiple_files', 'Tạo/sửa/xoá NHIỀU file cùng lúc trong 1 commit duy nhất (dùng khi cần thay đổi từ 2 file trở lên — sạch hơn nhiều so với gọi create_or_update_file lặp lại từng file, vì lặp lại sẽ tạo ra nhiều commit rời rạc)', {
    owner: { type: 'STRING' }, repo: { type: 'STRING' },
    message: { type: 'STRING', description: 'Nội dung commit message chung cho tất cả thay đổi' },
    branch: { type: 'STRING', description: 'Tên nhánh, để trống sẽ dùng nhánh mặc định của repo' },
    files: {
      type: 'ARRAY',
      description: 'Danh sách file cần thay đổi trong commit này',
      items: {
        type: 'OBJECT',
        properties: {
          path: { type: 'STRING', description: 'Đường dẫn file' },
          content: { type: 'STRING', description: 'Nội dung ĐẦY ĐỦ của file sau khi sửa — để trống hoặc bỏ qua nếu muốn XOÁ file này' },
          delete: { type: 'BOOLEAN', description: 'true nếu muốn xoá file này khỏi repo' },
        },
        required: ['path'],
      },
    },
  ...
  tool('list_check_runs', 'Xem trạng thái các CI check (pass/fail/pending) của 1 commit/nhánh/PR — dùng để biết PR có an toàn để merge hay không', {
    owner: { type: 'STRING' }, repo: { type: 'STRING' },
    ref: { type: 'STRING', description: 'SHA commit, tên nhánh, hoặc tag muốn xem check runs' },
  }, ['owner', 'repo', 'ref']),
  tool('list_workflow_runs', 'Liệt kê các lần chạy GitHub Actions workflow gần nhất của 1 repo', {
    owner: { type: 'STRING' }, repo: { type: 'STRING' },
    branch: { type: 'STRING', description: 'Chỉ lọc theo 1 nhánh, để trống lấy tất cả nhánh' },
    per_page: { type: 'INTEGER', description: 'Số lượng tối đa, mặc định 10' },
  }, ['owner', 'repo']),
...
async function githubApiFetch(token, path, options = {}) {
  ...
  if (!r.ok) {
    // Rate limit (chính hoặc "secondary" — spam quá nhanh) có message riêng dễ hiểu bằng tiếng
    // Việt thay vì để lộ message JSON thô của GitHub, và kèm thời gian còn phải đợi nếu có.
    const remaining = r.headers.get('x-ratelimit-remaining');
    const isPrimaryLimit = r.status === 403 && remaining === '0';
    const isSecondaryLimit = r.status === 403 && /secondary rate limit|abuse/i.test(data.message || '');
    if (isPrimaryLimit) {
      const resetEpoch = Number(r.headers.get('x-ratelimit-reset'));
      const waitMin = resetEpoch ? Math.max(1, Math.ceil((resetEpoch * 1000 - Date.now()) / 60000)) : null;
      throw new Error(`Đã đạt giới hạn số lượt gọi GitHub API cho phép trong giờ này${waitMin ? ` — thử lại sau khoảng ${waitMin} phút` : ''}.`);
    }
    if (isSecondaryLimit) {
      const retryAfter = r.headers.get('retry-after');
      throw new Error(`GitHub tạm chặn do gọi API quá nhanh liên tiếp${retryAfter ? ` — đợi khoảng ${retryAfter} giây` : ''} rồi thử lại.`);
    }
    throw new Error(`GitHub API lỗi ${r.status}: ${data.message || JSON.stringify(data).slice(0, 300)}`);
  }
  return data;
}
  ...
  try {
    switch (name) {
      ...
      case 'list_repo_contents': {
        ...
        const LIMIT = 200; // thư mục gốc của repo lớn có thể có hàng nghìn file -> tràn context Gemini
        const shown = list.slice(0, LIMIT);
        let text = shown.map(f => `${f.type === 'dir' ? '📁' : '📄'} ${f.path}`).join('\n');
        if (list.length > LIMIT) text += `\n... và ${list.length - LIMIT} mục khác (đã ẩn bớt, dùng "path" cụ thể hơn để thu hẹp).`;
        return { text };
      }
      ...
      case 'commit_multiple_files': {
        // Git Data API mức thấp: tạo 1 tree mới (chỉ chứa các file thay đổi, GitHub tự merge với
        // tree cũ) rồi 1 commit trỏ tới tree đó, rồi move nhánh sang commit mới — kết quả là ĐÚNG
        // 1 commit chứa mọi thay đổi, thay vì N commit rời rạc nếu gọi create_or_update_file lặp.
        const repoInfo = await githubApiFetch(token, `/repos/${args.owner}/${args.repo}`);
        const branch = args.branch || repoInfo.default_branch;
        const refData = await githubApiFetch(token, `/repos/${args.owner}/${args.repo}/git/ref/heads/${branch}`);
        const baseCommitSha = refData.object.sha;
        const baseCommit = await githubApiFetch(token, `/repos/${args.owner}/${args.repo}/git/commits/${baseCommitSha}`);

        const files = Array.isArray(args.files) ? args.files : [];
        if (!files.length) return { text: 'Không có file nào được truyền vào (files rỗng).', isError: true };

        // Mỗi file thay đổi cần 1 blob riêng (nội dung file) trước, trừ file bị xoá thì không cần
        // blob — dùng sha: null trong tree entry là cách Git báo "xoá đường dẫn này khỏi tree".
        const treeEntries = [];
        for (const f of files) {
          if (f.delete) {
            treeEntries.push({ path: f.path, mode: '100644', type: 'blob', sha: null });
          } else {
            const blob = await githubApiFetch(token, `/repos/${args.owner}/${args.repo}/git/blobs`, {
              method: 'POST', body: JSON.stringify({ content: toBase64Utf8(f.content || ''), encoding: 'base64' }),
            });
            treeEntries.push({ path: f.path, mode: '100644', type: 'blob', sha: blob.sha });
          }
        }

        const newTree = await githubApiFetch(token, `/repos/${args.owner}/${args.repo}/git/trees`, {
          method: 'POST', body: JSON.stringify({ base_tree: baseCommit.tree.sha, tree: treeEntries }),
        });
        const newCommit = await githubApiFetch(token, `/repos/${args.owner}/${args.repo}/git/commits`, {
          method: 'POST', body: JSON.stringify({ message: args.message, tree: newTree.sha, parents: [baseCommitSha] }),
        });
        await githubApiFetch(token, `/repos/${args.owner}/${args.repo}/git/refs/heads/${branch}`, {
          method: 'PATCH', body: JSON.stringify({ sha: newCommit.sha }),
        });

        return { text: `Đã tạo 1 commit gộp ${files.length} file thay đổi trên nhánh ${branch}: ${newCommit.sha.slice(0, 7)}` };
      }
      ...
      case 'merge_pull_request': {
        // GitHub tính "mergeable" bất đồng bộ (background job) — ngay sau khi tạo/cập nhật PR nó
        // có thể trả về null (đang tính) một lúc rồi mới có true/false thật. false thì CHẮC CHẮN
        // đang conflict, nên chặn lại ngay để tránh merge nhầm 1 PR có xung đột chưa giải quyết.
        const pr = await githubApiFetch(token, `/repos/${args.owner}/${args.repo}/pulls/${args.pull_number}`);
        if (pr.mergeable === false) {
          return { text: `Không thể merge PR #${args.pull_number}: đang có xung đột (conflict) với nhánh base, cần giải quyết conflict trước.`, isError: true };
        }
        if (pr.state !== 'open') {
          return { text: `Không thể merge PR #${args.pull_number}: PR đang ở trạng thái "${pr.state}", không phải "open".`, isError: true };
        }
      ...
      case 'list_check_runs': {
        const data = await githubApiFetch(token, `/repos/${args.owner}/${args.repo}/commits/${args.ref}/check-runs`);
        const runs = data.check_runs || [];
        if (!runs.length) return { text: `Không có CI check nào cho ${args.ref}.` };
        const icon = (r) => r.status !== 'completed' ? '⏳' : r.conclusion === 'success' ? '✅' : r.conclusion === 'skipped' || r.conclusion === 'neutral' ? '⏭️' : '❌';
        return { text: runs.map(r => `${icon(r)} ${r.name} — ${r.status}${r.conclusion ? `/${r.conclusion}` : ''}`).join('\n') };
      }
      case 'list_workflow_runs': {
        const q = args.branch ? `&branch=${encodeURIComponent(args.branch)}` : '';
        const data = await githubApiFetch(token, `/repos/${args.owner}/${args.repo}/actions/runs?per_page=${args.per_page || 10}${q}`);
        const runs = data.workflow_runs || [];
        return { text: runs.map(r => `#${r.run_number} ${r.name} (${r.head_branch}) — ${r.status}${r.conclusion ? `/${r.conclusion}` : ''}`).join('\n') || '(không có workflow run nào)' };
      }
