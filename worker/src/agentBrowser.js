...
export async function handleAct(id, body, env, json, cors) {
  try {
    ...
    return json({ id, url: result.url, pendingAction: result.pendingAction, log: result.log, screenshot: result.screenshot, result }, cors);
  } catch (err) { return json({ error: err.message }, cors, 500); }
}
...
export async function handleStep(id, body, env, json, cors, userId) {
  try {
    ...
    // Gộp "đọc trang" + "chụp ảnh" thành 1 lần kết nối trình duyệt duy nhất (xem readPageSummary).
    const summary = await agent.readPageSummary(env, id);
    const shot = summary.screenshot;
...
toán...) vẫn cứ đề xuất bình thường — hệ thống sẽ tự hỏi người dùng xác nhận.`;
    ...
    // Gộp "thực hiện hành động" + "chụp ảnh mới" thành 1 lần kết nối duy nhất (xem performAction).
    const result = await agent.performAction(env, id, decision.action, { force: false });
    return json({ thought: decision.thought, done: false, action: decision.action, needsApproval: result.needsApproval, screenshot: result.screenshot, url: result.url }, cors);
  } catch (err) { return json({ error: err.message }, cors, 500); }
}
