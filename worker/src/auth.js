...
const MAX_API_KEY_REGENS_PER_DAY = 2;

// Thu hồi key cũ (nếu có) và phát 1 key mới — dùng khi user lỡ để lộ key ra ngoài.
// Giới hạn MAX_API_KEY_REGENS_PER_DAY lần/NGÀY (tự reset sau 24h, không phải giới hạn vĩnh viễn)
// — dùng KV rate-limit sẵn có (kv.js) thay vì đếm cứng trong D1, để tự hết hạn theo thời gian.
async function regenerateApiKey(env, userId) {
  const db = env.MY_AI_DB;
  if (!db) throw new Error('D1 chưa được cấu hình');
  const ok = await checkRateLimit(env, `apikey-regen:${userId}`, { limit: MAX_API_KEY_REGENS_PER_DAY, windowSeconds: 86400 });
  if (!ok) {
    const err = new Error(`Bạn đã tạo key mới ${MAX_API_KEY_REGENS_PER_DAY} lần hôm nay — thử lại sau, hoặc liên hệ chủ app nếu cần gấp.`);
