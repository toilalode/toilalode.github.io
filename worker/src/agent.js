...
// Gọi generateContent nhưng tự thử lần lượt các key trong chain nếu key đang dùng bị hết hạn
// mức — dùng chung cho cả 3 hàm askJson/askJsonWithImage/askWithSearch bên dưới, vì Agent Mode
// gọi Gemini RẤT nhiều lần liên tiếp trong 1 tác vụ (mỗi bước 1 lần), càng cần key pool hơn cả chat.
async function geminiCallWithChain(env, keyChain, body) {
  if (!keyChain.length) throw new Error('Thiếu GEMINI_API_KEY (chưa cấu hình key nào cho tài khoản này)');
  let lastErr;
...
async function askWithSearch(env, keyChain, query) {
...
async function decideNextStep(env, keyChain, taskState) {
...
CHỈ dùng "browser_control" khi thực sự cần THAO TÁC trên trang, không chỉ đọc.`;
  return askJson(env, keyChain, prompt);
}

async function decideBrowserAction(env, keyChain, sessionId, subtask) {
...
Nếu việc con này đã xong, "done": true, điền "finalAnswer", "action": null.`;
  const decision = await askJsonWithImage(env, keyChain, prompt, base64Data);
  return { decision, shot };
}

async function runLoop(taskState, env, keyChain, send) {
  let steps = taskState.steps.length;
  while (steps < MAX_STEPS) {
    steps++;
    const decision = await decideNextStep(env, keyChain, taskState);
    ...
    if (decision.tool === 'search_web') {
      ...
      const text = await askWithSearch(env, keyChain, decision.args?.query || taskState.task);
    ...
    if (decision.tool === 'browse_url') {
      ...
      const answer = await askWithSearch(env, keyChain, `Nội dung trang ${url}:\n"""${text}"""\n\nCâu hỏi: ${decision.args?.question || 'Tóm tắt nội dung chính.'}`);
    ...
    if (decision.tool === 'browser_control') {
      ...
      const { decision: bdec, shot } = await decideBrowserAction(env, keyChain, taskState.browserSessionId, subtask);
...
export function handleRun(body, env, cors, ctx, userId) {
  return sseResponse(cors, async (send) => {
    const keyChain = await getGeminiKeyChain(env, userId);
    if (keyChain.length === 0) { await send('error', 'Thiếu GEMINI_API_KEY'); return; }
    const { task } = body || {};
    ...
    await runLoop(taskState, env, keyChain, send);
  }, ctx);
}

// POST /api/agent/:id/resume { approve }
export function handleResume(id, body, env, cors, ctx, userId) {
  return sseResponse(cors, async (send) => {
    const keyChain = await getGeminiKeyChain(env, userId);
    let taskState;
