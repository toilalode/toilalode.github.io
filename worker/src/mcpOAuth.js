...
// ===================== OAUTH CHO MCP (GitHub) =====================
// Luồng chuẩn Authorization Code: /oauth/start -> redirect người dùng sang trang đăng nhập của
// provider -> họ đồng ý -> provider redirect về /oauth/callback kèm "code" -> worker đổi code
// lấy access_token -> lưu vào D1 (user_mcp_tokens), gắn với user đang đăng nhập (biết được nhờ
// "state" — 1 chuỗi ngẫu nhiên lưu tạm trong KV, ánh xạ sang userId).
//
// Cấu hình cần thiết (wrangler secret put ... hoặc dashboard):
//   GITHUB_OAUTH_CLIENT_ID / GITHUB_OAUTH_CLIENT_SECRET
//     -> Tạo tại: github.com/settings/developers -> "New OAuth App"
//     -> Authorization callback URL PHẢI đúng: <API_BASE>/api/mcp/oauth/callback
//
// (Gmail/Drive/Notion/Slack đã BỎ — Gmail/Drive cần tài khoản Google Workspace trả phí + được
// duyệt Developer Preview Program mới gọi được thật; Notion/Slack cần đăng ký OAuth App riêng
// chưa làm. Có thể thêm lại sau vào OAUTH_PROVIDERS bên dưới nếu cần, không phải sửa gì khác.)
