...
async function getCachedToolDeclarations(env, provider, url, token, isNative) {
  const cacheKey = `mcp-tools-cache:${provider}`;
...
async function runMcpChat(env, keyChain, servers, userMessage) {
  ...
  const sessions = {}; // provider -> { sessionId (nếu là MCP thật), native, url, token }
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
    ...
    if (!route || !sess) {
      ...
      try {
        ...
        toolResult = await mcpCallTool(sess.url, sess.token, sessionId, route.originalName, functionCall.args);
      } catch (e) {
